...
def can_generate_allowlist_certificate(user, course_key):
    ...
    if enrollment_mode is None:
        log.info(u'{user} : {course} does not have an enrollment. Certificate cannot be generated.'.format(
            user=user.id,
            course=course_key
        ))
        return False
    ...
    if not IDVerificationService.user_is_verified(user):
        log.info(
            u'{user} does not have a verified id. Certificate cannot be generated.'.format(
                user=user.id
            ))
        return False
